...
export interface FocusViewViewProps {
  listing: Listing;
  ...
  /**
   * Leave focus mode. The connected half clears the selection and returns the
   * camera; escape and the × both route here.
   */
  onClose(): void;
}
 ...
 */
export function FocusViewView({
  listing, engine, size, onOpen, onClose,
}: FocusViewViewProps) {
  ...
  return (
    <div ref={wrapperRef} className="pointer-events-none absolute inset-0">
      ...
      <div className="pointer-events-auto absolute left-3 top-3 z-20 md:left-4 md:top-4">
        <IconButton tone="ghost" label={t.backToList} onClick={onClose}>
          ×
        </IconButton>
      </div>
